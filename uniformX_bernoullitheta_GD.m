...
f1=@(xv,i) (1/(b-a))*pth(i); % pdf of (X,theta)
for M=Mval

xrandinit=reshape(xinitializations(find(M==Mv),:,1:M+1,:),[length(thval),M+1,rn]); % initializations
...
% display output
disp(strcat('M = ',num2str(M),', r = ',num2str(r))) % number of levels of quantization, initialization number, correlation 
exitf % exit flag
xm % quantizer
...
disp(strcat('Results: M = ',num2str(M)))
...
% save to mat file
save(strcat('uniform_xthetaM',num2str(M),'rho0a',num2str(a),'b',num2str(b),'noiseless_binarytheta_GD.mat'),'xm','ym','dist_enc','dist_dec','erm','xrm','yrm','drm','derend','xrandinit')
...
num=f1(xm(t,i),t);
for th=1:size(thval)
    den1=den1+integral(@(xv) f1(xv,th),xm(th,i-1),xm(th,i),'ArrayValued',true);
    den2=den2+integral(@(xv) f1(xv,th),xm(th,i),xm(th,i+1),'ArrayValued',true);
end
